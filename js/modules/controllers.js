...
if(localStorage.getItem("role")== "judgment"){
    ...
    $scope.checked='judgment';
    $scope.user = {NationalNumber : localStorage.getItem('judgment')};
}else if(localStorage.getItem('role') == "candidate"){
$scope.checked='candidate';
$scope.user  = {NationalNumber : localStorage.getItem('candidate_nationalId')};
}
...
if(result){
    Web3jsObj.createBrainWallet(user.NationalNumber, user.password).then(function(_wallet){
        ...
        localStorage.setItem('judgment',user.NationalNumber)
...
$scope.user  = {adminEmail :localStorage.getItem("admin")};    
